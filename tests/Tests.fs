...
let ``test transaction parsing`` () =
    ...
    Assert.Equal(1, tx.TxIns.Length)
    let want = bytes_from_hex "d1c789a9c60383bf715f3f6ad9d14b91fe55f3deb369fe5d9280cb1a01793f81"
    Assert.True((want = tx.TxIns[0].PrevTx))
    Assert.Equal(tx.TxIns[0].PrevIndex, 0u)
    let want = bytes_from_hex "6b483045022100ed81ff192e75a3fd2304004dcadb746fa5e24c5031ccfcf21320b0277457c98f02207a986d955c6e0cb35d446a89d3f56100f4d7f67801c31967743a9c8e10615bed01210349fc4e631e3624a545de3f89f5d8684c7b8138bd94bdd531d2e213bf016b278a"
    Assert.True((tx.TxIns[0].ScriptSig.Serialize = want))
    Assert.Equal(tx.TxIns[0].Sequence, 0xfffffffeu)

    Assert.Equal(2, tx.TxOuts.Length)
    Assert.Equal(tx.TxOuts[0].Amount, 32454049UL)
    let want = bytes_from_hex "1976a914bc3b654dca7e56b04dca18f2566cdaf02e8d9ada88ac"
    Assert.True((tx.TxOuts[0].ScriptPubKey.Serialize = want))
    Assert.Equal(tx.TxOuts[1].Amount, 10011545UL)
    let want = bytes_from_hex "1976a9141c4bc762dd5423e332166702cb75f40df79fea1288ac"
    Assert.True((tx.TxOuts[1].ScriptPubKey.Serialize = want))

    Assert.Equal(tx.Locktime, 410393u)
    Assert.True((tx.Serialize = raw_tx ))

[<Fact>]
let ``test transaction input value`` () =
    let tx_hash = "d1c789a9c60383bf715f3f6ad9d14b91fe55f3deb369fe5d9280cb1a01793f81"
    let tx_in = TxIn.Create(bytes_from_hex tx_hash, 0u)
    Assert.Equal(42505594UL, TxHelper.get_output_value  tx_in false)
    let want = bytes_from_hex "1976a914a802fc56c704ce87c42d7c92eb75e7896bdc41ae88ac"
    Assert.True((want = (TxHelper.get_script_pubkey tx_in false).Serialize))
...
let ``test transaction fee`` () =
    ...
    use stream = new MemoryStream(raw_tx)
    let tx = Tx.Parse stream
    Assert.Equal(40000UL, TxHelper.get_fee tx false)

    let raw_tx = bytes_from_hex ""
    use stream = new MemoryStream(raw_tx)
    let tx = Tx.Parse stream
    Assert.Equal(140500UL, TxHelper.get_fee tx false)

[<Fact>]
let ``test verify p2pkh`` () =
    let tx = TxHelper.fetch "452c629d67e41baec3ac6f04fe744b4b9617f8f859c63b3002f8684e7a4fee03" false false
...
[<Fact>]
let ``test sign input`` () =
    let private_key = PrivateKey.Create 8675309
    let tx_bytes = bytes_from_hex "010000000199a24308080ab26e6fb65c4eccfadf76749bb5bfa8cb08f291320b3c21e56f0d0d00000000ffffffff02408af701000000001976a914d52ad7ca9b3d096a38e752c2018e6fbc40cdf26f88ac80969800000000001976a914507b27411ccf7f16f10297de6cef3f291623eddf88ac00000000"
    use stream = new MemoryStream(tx_bytes)
    let tx = Tx.Parse(stream, true)
    let signed, tx = TxHelper.sign_input tx 0 private_key true
    Assert.True <| signed
    ...
    Assert.Equal(want, bytes_to_hex tx.Serialize)

[<Fact>]
let ``test is coinbase`` () =
    let raw_tx = bytes_from_hex ""
    use stream = new MemoryStream(raw_tx)
    let tx = Tx.Parse stream
    Assert.True tx.IsCoinbase

[<Fact>]
let ``test coinbase height`` () =
    let raw_tx = bytes_from_hex ""
    use stream = new MemoryStream(raw_tx)
    let tx = Tx.Parse stream
    match tx.CoinbaseHeight with
    | Some h -> Assert.Equal(465879UL, h)
    | None -> Assert.True false
    let raw_tx = bytes_from_hex ""
    use stream = new MemoryStream(raw_tx)
    let tx = Tx.Parse stream
    Assert.True tx.CoinbaseHeight.IsNone
...
// [<Fact>]
// let ``test transaction fee long network`` () =
...
//     let target_output = TxOut.Create(target_amount, target_script)
//     // let tx = Tx.Create(1u, [| tx_in |], [| change_output; target_output |], 0u)
//     let tx = Tx.Create(1u, [| tx_in |], [| target_output; change_output |], 0u, true)
//     let secret = 8675309
...
let ``test data data message serialization`` () =
    ...
    get_data.AddData DataType.MSG_FILTERED_BLOCK block1
    let block2 = bytes_from_hex "00000000000000beb88910c46f6b442312361c6693a7fb52065b583979844910"
    get_data.AddData DataType.MSG_FILTERED_BLOCK block2
