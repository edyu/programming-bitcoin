...
let ``test verification of signature`` () =
    ...
    let px = bigint_fromhex("04519fac3d910ca7e7138f7013706f619fa8f033e6ec6e09370ea38cee6a7574")
    let py = bigint_fromhex("82b51eab8c27c66e26c858a079bcdf4f1ada34cec420cafc7eac1a42216fb6c4")
    let point = S256Point.Create px py
    Assert.True(point.verify z { r = r; s = s })

[<Fact>]
let ``test verification of signature 1`` () =
    let z = bigint_fromhex("0xec208baa0fc1c19f708a9ca96fdeff3ac3f230bb4a7ba4aede4942ad003c0f60")
    let r = bigint_fromhex("0xac8d1c87e51d0d441be8b3dd5b05c8795b48875dffe00b7ffcfac23010d3a395")
    let s = bigint_fromhex("0x68342ceff8935ededd102dd876ffd6ba72d6a427a3edb13d26eb0781cb423c4")
    let px = bigint_fromhex("887387e452b8eacc4acfde10d9aaf7f6d9a0f975aabb10d006e4da568744d06c")
    let py = bigint_fromhex("61de6d95231cd89026e286df3b6ae4a894a3378e393e93a0f45b666329a0ae34")
    let point = S256Point.Create px py
    Assert.True(point.verify z { r = r; s = s })

[<Fact>]
let ``test verification of signature 2`` () =
    let z = bigint_fromhex("0x7c076ff316692a3d7eb3c3bb0f8b1488cf72e1afcd929e29307032997a838a3d")
    let r = bigint_fromhex("0xeff69ef2b1bd93a66ed5219add4fb51e11a840f404876325a1e8ffe0529a2c")
    let s = bigint_fromhex("0xc7207fee197d27c618aea621406f6bf5ef6fca38681d82b2f06fddbdce6feab6")
    let px = bigint_fromhex("887387e452b8eacc4acfde10d9aaf7f6d9a0f975aabb10d006e4da568744d06c")
    let py = bigint_fromhex("61de6d95231cd89026e286df3b6ae4a894a3378e393e93a0f45b666329a0ae34")
    let point = S256Point.Create px py
    Assert.True(point.verify z { r = r; s = s })
